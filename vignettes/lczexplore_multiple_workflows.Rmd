...
## Visual exploration of multiple maps
Pairwise visualisation can be tedious, especially when several locations are involved.
The sf object containing all the intersected spatial units and the LCZ type each workflow assigned them allows to
begin the comparison.

### Area per LCZ type and workflow

The function `barplotLCZfromIntersected` allows the visualisation of what area is classified into each LCZ type
of each workflow.
```{r, barplotLCZfromIntersected}
areaPerLCZtype<-barplotLCZfromIntersect(twoLocsSfIntersected
