...
# Multiple workflows and one or multiple locations
...
From a main directory, data are stored in subdirectories named after locations. In each subdirectory, the
...
The function loadMultipleSfs allows to load the lcz maps files from a subdirectory into a list. In the data structure
from above section, the function would be applied one of the `location1` and `location2` directories.

```{r loadMultiple}
library(lczexplore)
blaruDirPath<-paste0(system.file("extdata/multipleWfs/Blaru", package="lczexplore"),"/")
blaruSfList<-loadMultipleSfs(dirPath = blaruDirPath, workflowNames = c("osm","bdt","iau","wudapt"), fileExtension =".fgb" )
```

To concatenate the sf objects from the list returned by `loadMultipleSfs`, use `concatAlocationWorkflows`.
The sf objects contained in the list you feed `concatAlocationWorkflows` may not all have the same CRS. By defautl,
...
from above section, the function would be applied to the `main_directory` folder.
