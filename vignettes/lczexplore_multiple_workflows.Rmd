...
*Intersection.* Second, the spatial units of all maps (i.e. the geometries bearing the LCZ type) are intersected.
...
*Visualisation* Simple descriptive barplots allow to have a general idea of the behavior of the different workflows.
*Comparison* Pairwise agreement between workflow and consensus value for each LCZ type quantify comparison.
...
To explore this agreement further, we compute all the repartition from an LCZ type from a workflow to the LCZ types of another workflow, using the function `createMultipleMatConf`

```{r, multiple confusion matrix}
twoLocsMatConfLong<-createMultipleMatConf(twoLocsSfIntersected, wfNamesIn = c("osm","bdt","wudapt"))
```
